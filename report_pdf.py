...
def _header_footer(canvas, doc):
    ...
    canvas.drawString(20, 6, "Quantum-Ready Cybersecurity for Future-Safe Banking  ·  INTERNAL — CONFIDENTIAL")
...
def generate_pdf(
    ...
    # ============================================================
    # SECTION 6 — Digital Certificates of Compliance
    # ============================================================
    for ep in endpoints:
        pqc_cert = ep.get("pqc_certificate") or {}
        if not pqc_cert:
            continue
        
        cert_details = pqc_cert.get("certificate") or {}
        sig = pqc_cert.get("signature") or ""
        cert_status = cert_details.get("status", "Unknown")
        
        story.append(PageBreak())
        story.append(Spacer(1, 1 * cm))
        
        # Draw a beautiful certificate border using a table with padding
        ...
        cert_status_style = ParagraphStyle(
            "CertStatus",
            parent=T,
            fontSize=14,
            textColor=colors.HexColor("#388E3C") if "Safe" in cert_status or "Ready" in cert_status else colors.HexColor("#D32F2F"),
            alignment=1, # Center
            fontName="Helvetica-Bold",
            spaceAfter=15,
        )
        
        cert_slogan_style = ParagraphStyle(
            "CertSlogan",
            parent=Body,
            fontSize=9.5,
        ...
        cert_elements = [
            Paragraph("Q-SHIELD CYBERSECURITY COMPLIANCE CERTIFICATE", cert_title_style),
            Paragraph("Post-Quantum Readiness & Cryptographic Validation", cert_subtitle_style),
            Paragraph("This is to certify that the public-facing endpoint", cert_body_style),
            Paragraph(f"<b>{ep.get('endpoint', 'Unknown')}</b>", cert_title_style),
            Paragraph("has been scanned and evaluated against post-quantum security criteria.", cert_body_style),
            Paragraph("COMPLIANCE STATUS LABEL:", cert_body_style),
            Paragraph(f"[{cert_status.upper()}]", cert_status_style),
            Paragraph(f"“{cert_details.get('slogan', 'Quantum-Ready Cybersecurity for Future-Safe Banking')}”", cert_slogan_style),
            Spacer(1, 0.4 * cm),
            Paragraph(f"<b>Verified Algorithms:</b> {', '.join(cert_details.get('algorithms', [])) or 'None (Classical Only)'}", cert_body_style),
            Paragraph(f"<b>Issued by:</b> {cert_details.get('issuer', 'QShield Certificate Authority')} on {cert_details.get('certified_at', 'N/A')[:10]}", cert_body_style),
            Paragraph(f"<b>Valid until:</b> {cert_details.get('valid_until', 'N/A')[:10]}", cert_body_style),
            Spacer(1, 0.4 * cm),
            Paragraph(f"<b>HMAC SHA-256 DIGITAL SIGNATURE (VERIFIED):</b><br/>{sig}", cert_sig_style),
        ]
        
        # Wrap in an elegant framed table with gold border
        cert_table_data = [[cert_elements]]
        cert_table = Table(cert_table_data, colWidths=[16.2 * cm])
        cert_table.setStyle(TableStyle([
            ("BOX", (0, 0), (-1, -1), 2.5, colors.HexColor("#D4AF37")), # Gold border
            ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#FCFBF7")), # Parchment bg
